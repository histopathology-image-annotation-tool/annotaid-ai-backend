...
from .layers import Conv_Bn_Relu, Multiscale_Conv_Block, Residual_Conv
...
class NuClick_NN(nn.Module):
    ...
    def __init__(
        ...
        self.conv_block_3 = Conv_Bn_Relu(
            in_channels=32, out_channels=self.n_classes, kernelSize=(1, 1),
            actv=None, useBias=True, doBatchNorm=False)
        ...
        self.multiscale_block_1 = Multiscale_Conv_Block(
            in_channels=128, out_channels=32, kernelSizes=[3, 3, 5, 5],
            dilatationRates=[1, 3, 3, 6],
            isDense=False)

        self.multiscale_block_2 = Multiscale_Conv_Block(
            in_channels=256, out_channels=64, kernelSizes=[3, 3, 5, 5],
            dilatationRates=[1, 3, 2, 3],
            isDense=False)

        self.multiscale_block_3 = Multiscale_Conv_Block(
            in_channels=64, out_channels=16, kernelSizes=[3, 3, 5, 7],
            dilatationRates=[1, 3, 2, 6],
            isDense=False)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        up61 = torch.cat([self.conv_transpose_1(conv51), conv5],
                         dim=1)  # up61: 1024 channels
        conv61 = self.residual_block_7(up61)  # conv61: 256 channels

        up6 = torch.cat([self.conv_transpose_2(conv61), conv4],
                        dim=1)  # up6: 512 channels
        ...
        up7 = torch.cat([self.conv_transpose_3(conv6), conv3],
                        dim=1)  # up7: 256 channels
        conv7 = self.residual_block_10(up7)  # conv7: 128 channels

        up8 = torch.cat([self.conv_transpose_4(conv7), conv2],
                        dim=1)  # up8: 128 channels
        ...
        up9 = torch.cat([self.conv_transpose_5(conv8), conv1],
                        dim=1)  # up9: 64 channels
